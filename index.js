...
const secret_key =
  "sk_test_51OufVpSGE6lcza02hUWVWuuAakF5ZYjU79eT6KDFLyraYSuUYSHivLfnZol8lTHARrCneyzHfw91v3jqOxw4xhjr00Hpw1851R";
...
app.post("/changepassword", (req, res) => {
  ...
  if (req.session.position=="user") {
    userdata.findOne({ username: username }).then((response) => {
      if (
        req.body.newpass1 !== req.body.newpass2 ||
        response.password !== req.body.oldpass
      ) {
...
app.post("/deleteaccount", (req, res) => {
  ...
  userdata.findOne({ username: username }).then((response) => {
    if (
      ...
      response.password !== req.body.pass1
    ) { 
      ...
      productdata
        ...
        .then((response1) => {
          ...
          if(totalPrice>100000){
            req.session.cartOverprisec=true;
            return res.redirect("/");
          }
...
app.post("/payment", async (req, res) => {
  if (req.session.username) {
    let { firstname, lastname, address, country, zipcode, city, state, phone } =
      req.body;
    ...
    let user = await userdata.findOne({ username: req.session.username });
    let product = await productdata.findOne({
...
app.get("/orderplaced", async (req, res) => {
  ...
  let product = await productdata.findOne({ _id: new ObjectId(req.query.id) });
  product.address = req.session.address;
  let orderDetails = await orderdata.findOne({
...
app.get("/pay", async (req, res) => {
  ...
  res.render("paygetway", {
    key: publish_key,
    product: product,
  });
});
app.post("/paymentgateway", async (req, res) => {
  let product = await productdata.findOne({ _id: new ObjectId(req.body.id) });
  let add = req.session.address;
  stripe.customers
    .create({
...
app.get("/category", async (req, res) => {
  ...
  let product = await productdata
    ...
    .toArray();
  res.render("index", { product: product, user: user ? user : "" });
});

app.get("/search", async (req, res) => {
  let user = await userdata.findOne({ username: req.session.username });
  let product1 = await productdata
    .find({ name: req.query.searchItem })
    .toArray();
  let product2 = await productdata
    .find({ category: req.query.searchItem })
    .toArray();

  res.render("index", { product: [...product1,...product2], user: user ? user : "" });
});
  ...
  if (position === "user") {
    if (req.file) {
      await userdata.updateOne(
        { username: username },
        { $set: { url: "/uploads/user/" + req.file.filename } }
      );
    }
    ...
    if (req.file) {
      await admindata.updateOne(
        { username: username },
        { $set: { url: "/uploads/admin/" + req.file.filename } }
      );
    }
    if (req.username) {
      await admindata.updateOne(
...
app.post("/addproduct", product.single("image"), async (req, res) => {
  // if(!req.body.productname && !req.body.description&&!req.body.price&&!req.body.category){
...
app.get("/admin/deleteproduct",(req,res)=>{
  if(req.session.username && req.session.position=="admin"){
    productdata.deleteOne({"_id":new ObjectId(req.query.id)}).then(()=>{
      res.redirect("/admin/editproduct");
    })
  }else{
    res.redirect("/admin/login");
